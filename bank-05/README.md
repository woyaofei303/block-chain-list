...
3. 上述两个提交已推送到 GitHub 的 `main` 分支。[查看 Bank 项目](https://github.com/woyaofei303/block-chain-list/tree/main/bank-05)

首次提交时，截图只存在 Git 忽略的 `output-tdd/` 目录，README 链接在仓库中无法显示。修复时把用于展示的图片复制到 `bank-05/screenshots/`，用相对路径嵌入 README，再将图片一起提交。原始 RPC 等临时产物继续留在本地。

后续更新时，可在仓库根目录按下面顺序检查和提交。这些是操作示例，不表示本文新增内容已经提交或推送：

```sh
git status --short
git diff -- bank-05/README.md bank-05/contracts/Bank.sol bank-05/tests/BankDepositor.sol bank-05/tests/Bank_test.sol
git add -- bank-05/README.md bank-05/contracts/Bank.sol bank-05/tests/BankDepositor.sol bank-05/tests/Bank_test.sol bank-05/screenshots
