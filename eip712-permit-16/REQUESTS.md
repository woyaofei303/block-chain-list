...
本项目复用第 13 题的 TypeScript、TanStack Query v5、Viem、Express、PostgreSQL、`p-queue` 与 Sonner。先读 [AGENTS.md](AGENTS.md)，运行流程见 [WALKTHROUGH.md](WALKTHROUGH.md)。
...
function permitDeposit(uint256 amount, bytes32 operationId, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external;
...
摘要是 `keccak256(abi.encode(isDeposit, amount))`。同账户、同编号、同参数重放直接返回，不再次转币或产生业务事件；不同参数回滚。标记先于外部转币写入，资金转移失败时整笔交易回滚，原编号可重试；重入保护覆盖全部存取款入口。新合约没有不带编号的入口。仅支持标准无手续费、无 rebase 的 Token，例如 BaseERC20 与第 16 题 JulianToken。
...
新环境完整步骤见 WALKTHROUGH，部署目标已经改为 IdempotentTokenBank。后端新增配置：
...
本项目使用 `8547 / 13016 / 3016` 的独立环境，不迁移历史余额和数据库。已有部署不能自动升级；不要一边运行同目录服务一边构建。
