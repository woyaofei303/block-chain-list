...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): AppConfig {
  const rpcUrl = env.RPC_URL ?? 'http://127.0.0.1:8547'
  if (
    !URL.canParse(rpcUrl) ||
  ...
  const publicOrigin = env.PUBLIC_ORIGIN ?? 'http://127.0.0.1:3016'
  if (
