...
import { createOperationsRouter } from './operations/router.ts'
import { scanOnce } from './transfers/indexer.ts'
import { initDatabase } from './transfers/repository.ts'
...
try {
  ...
  if (config.bankAddress) {
    const bankToken = await rpc.readContract({
      address: config.bankAddress,
      abi: parseAbi(['function token() view returns (address)']),
      functionName: 'token',
    })
    if (bankToken.toLowerCase() !== config.tokenAddress.toLowerCase())
      throw new Error('Bank token mismatch')
  ...
  if (process.argv.includes('--once')) {
    ...
    const operations = config.bankAddress
      ? createOperationsRouter(db, rpc, {
          ...config,
          bankAddress: config.bankAddress,
        })
      : undefined
    const app = createApp(db, config, { symbol, decimals }, operations)
