...
需要连同钱包和前端一起验收时，参照 [Token Bank 完整实操流程](../tokenbank-fullstack-13/WALKTHROUGH.md)，从本地合约部署、测试资产准备到 API 和数据库核对逐步执行。
...
默认值来自仓库 `foundry-counter-09/broadcast/MyToken.s.sol/11155111/run-latest.json`：
...
```bash
cd /Users/julian/Documents/Codex/2026-09-03/block-chain-list/erc20-event-indexer-12
npm ci
npm start
```
