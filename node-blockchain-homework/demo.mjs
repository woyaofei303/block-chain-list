...
async function poll(predicate, description) {
  ...
  throw new Error(`${description} 超时（${TIMEOUT_MS} ms）${lastError ? `: ${lastError.message}` : ""}`)
}
...
try {
  ...
  await poll(async () => {
    const [statusA, statusB] = await Promise.all([
      requestJson(`${httpA}/status`),
      requestJson(`${httpB}/status`),
    ])
  ...
  await poll(
    async () => (await requestJson(`${httpB}/mempool`)).transactions.some((transaction) => transaction.id === second.transaction.id),
    "交易广播"
  )
