...
先阅读 [账户、代币、银行和请求流程](README.md#先理解账户代币和银行)，再选择下面的入口：
...
pnpm --dir tokenbankv2/frontend dev --port 3180
```
...
本机仓库路径如下；换机器时，将后续命令中的这一段替换成你的仓库根目录。

```text
/Users/julian/Documents/Codex/2026-09-03/block-chain-list
```
...
```bash
npm --prefix erc20-event-indexer ci
pnpm --dir tokenbankv2/frontend install --frozen-lockfile
```
...
forge create tokenbank/contracts/BaseERC20.sol:BaseERC20 \
  --root . --contracts tokenbank/contracts \
  --out output-tdd/tokenbank-walkthrough/out \
  --cache-path output-tdd/tokenbank-walkthrough/cache \
...
forge create tokenbank/contracts/TokenBank.sol:TokenBank \
  --root . --contracts tokenbank/contracts \
  --out output-tdd/tokenbank-walkthrough/out \
  --cache-path output-tdd/tokenbank-walkthrough/cache \
  --rpc-url "$RPC_URL" --from "$TOKENBANK_DEPLOYER" \
...
source output-tdd/tokenbank-walkthrough/session.env
set +a
pnpm --dir tokenbankv2/frontend dev --port 3181
```
...
保持连接刚才存款的同一个钱包，选择“取出”，输入 `4`，点击“取出 Token”并在钱包确认。取款不需要再授权，也不需要填写接收地址；银行会自动退给发起取款的当前钱包。若当前是另一个没有存款的账户，切回原账户即可，银行合约地址不需要改。
...
```bash
pnpm --dir tokenbankv2/frontend lint
pnpm --dir tokenbankv2/frontend format:check
pnpm --dir tokenbankv2/frontend typecheck
pnpm --dir tokenbankv2/frontend test
pnpm --dir tokenbankv2/frontend test:integration
```

普通测试目前有 4 项，覆盖钱包拒绝请求的提示、精确金额、接口响应校验和 Next.js 同源代理。集成测试有 1 项，自动启动独立 Anvil 并验证授权、存取款、拒签及账户 / 网络变化；结束后关闭该测试链，不依赖上面运行的 A，也不保留可供页面使用的合约。

索引器也有自己的检查：

```bash
npm --prefix erc20-event-indexer test
npm --prefix erc20-event-indexer run lint
npm --prefix erc20-event-indexer run format:check
```
...
source output-tdd/tokenbank-walkthrough/session.env
set +a
pnpm --dir tokenbankv2/frontend build
pnpm --dir tokenbankv2/frontend start --port 3181
```
...
3. 按 [索引器 README](../../erc20-event-indexer/README.md) 填写后端 `.env`，然后运行 `npm start`。
4. 按 [前端 README](README.md) 填写 `.env.local`，将 `INDEXER_URL` 指向这次启动的索引器。开发模式重启，生产模式重新构建再启动。
...
- **`cast wallet list` 没有 Anvil 的账户**：它只列出 Foundry 本机密钥库。Anvil 提供的账户不会自动导入；`eth_accounts` 也只返回地址，不返回私钥。推荐按第 4.2 节给现有 MetaMask 账户充值，账户来源区别见 [README](README.md#anvilmetamask-和-foundry-账户有什么区别)。
